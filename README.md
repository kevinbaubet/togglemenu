...
Lors de l'initialisation d'un display, ToggleMenu va executer une classe JS qui correspondra à **$.ToggleMenuName**. "Name" étant le nom du display.
...
Cependant, chaque display doit avoir la possiblité d'être retiré de la page. Lors d'un appel à setDisplay(), si un display est déjà initilaisé, ToggleMenu va executer la méthode unload() du display initialisé avant d'en charger un nouveau.
